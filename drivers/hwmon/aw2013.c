...
static ssize_t aw2013_reg_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct aw2013_control_data *led;
	struct led_classdev *led_cdev = dev_get_drvdata(dev);
	...
	led = container_of(led_cdev, struct aw2013_control_data, cdev);
	ret = aw2013_i2c_rx_byte_data(led->i2c_client,aw2013_regs.soft_reset,&regdata);
	count += sprintf(buf+count, "[%x] = (%x)\n", aw2013_regs.soft_reset, regdata);
	ret = aw2013_i2c_rx_byte_data(led->i2c_client,aw2013_regs.enable_led,&regdata);
	count += sprintf(buf+count, "[%x] = (%x)\n", aw2013_regs.enable_led, regdata);
	ret = aw2013_i2c_rx_byte_data(led->i2c_client,aw2013_regs.irq_state,&regdata);
	count += sprintf(buf+count, "[%x] = (%x)\n", aw2013_regs.irq_state, regdata);

	for (i = 0x30; i < 0x40; i++)
	{
		ret = aw2013_i2c_rx_byte_data(led->i2c_client, 0x00+i, &regdata);
		if (ret < 0)
			break;
		else
			count += sprintf(buf+count, "[%x] = (%x)\n", 0x00+i, regdata);
...
static ssize_t aw2013_reg_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
	struct aw2013_control_data *led;
	struct led_classdev *led_cdev = dev_get_drvdata(dev);
	int addr, cmd;
...
static DEVICE_ATTR(reg_debug, 0664, aw2013_reg_show, aw2013_reg_store);
...

{
	...
	while(count--) {
		...
		if  (ret >= 0) {
			dev_info(&client->dev,"aw2013_debug: reg[0x02] = %d\n", ret);
			break;
		}
	...
	write_reg(aw2013_data, 0x01, 0x01);
       aw2013_data->aw2013_ic_suspend = 0;
...
static int aw2013_suspend(struct i2c_client *cl, pm_message_t mesg)
{
    ...
    if(!(regdata & 0x07) && !aw2013_data->aw2013_ic_suspend)
    {
         write_reg(aw2013_data, 0x01, 0x00);// for aw2013 suspend setting GCR Register 0
         aw2013_data->aw2013_ic_suspend = 1;
    }

    LED_DEBUG("ztemt_aw2013_suspend aw2013_ic_suspend=%d \n",aw2013_data->aw2013_ic_suspend);
    return 0;
};
...
static int aw2013_resume(struct i2c_client *cl)
{
    ...
    if(aw2013_data->aw2013_ic_suspend)
    {
          write_reg(aw2013_data, 0x01, 0x01);// for aw2013 resume setting GCR Register 1
          aw2013_data->aw2013_ic_suspend = 0;
    }

     LED_DEBUG("ztemt_aw2013_resume aw2013_ic_suspend=%d \n",aw2013_data->aw2013_ic_suspend);
     return 0;
};
